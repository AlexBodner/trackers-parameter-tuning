<h1> Extracted YOLOX detections for the <u>MOT17</u> dataset </h1>

Related notebooks:
<ul> 
<li>ByteTrack runs on MOT17 sequences: <a href="https://colab.research.google.com/drive/1Z8zSmq1_VZ7IBJWf5WpFt8nId6UxJwIi">link</a></i>
<li>ByteTrack evaluation on MOT17 sequences: <a href="https://colab.research.google.com/drive/1eh-2AuSbsntGcfiO5Vwns2Zw4zQa8_us">link</a></i>
</ul>
...
Note that due to the standards assumed in the community, the MOT17-val set consists of second halves of the training sequences. Therefore, the frame_numbers in `MOT17/val` files do not start with 1, but with the specific numbers. Accordingly with the community evaluation standards, these numbers are as follows:
<ul>
<li>MOT17-02 val: from 302</li>
<li>MOT17-04 val: from 527</li>
<li>MOT17-05 val: from 420</li>
<li>MOT17-09 val: from 264</li>
<li>MOT17-10 val: from 329</li>
<li>MOT17-11 val: from 452</li>
<li>MOT17-13 val: from 377.</li>
</ul>
<br/>
...
MOT17 train and test: `-f exps/example/mot/yolox_x_mix_det.py -c pretrained/bytetrack_x_mot17.pth.tar` <br/>
MOT17 val: `-f exps/example/mot/yolox_x_ablation.py -c pretrained/bytetrack_ablation.pth.tar` <br/>
<br/>
E.g. for MOT17 test set, a single sequence detection extracting command would be:<br/>

`python3 tools/demo_track__only_save_dets.py image -f exps/example/mot/yolox_x_mix_det.py -c pretrained/bytetrack_x_mot17.pth.tar --fp16 --conf 0.01 --nms 0.7 --path /my_dir/datasets/MOT17/test/MOT17-01-DPM/img1`

<h2> Pre-trained models </h2>
The pre-trained detection models are available here:<br/>
<a href="https://github.com/FoundationVision/ByteTrack?tab=readme-ov-file#model-zoo">MOT17</a> - bytetrack_x_mot17.pth.tar and bytetrack_ablation.pth.tar<br/>
