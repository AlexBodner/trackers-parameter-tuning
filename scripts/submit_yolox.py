...
DATASET_DET_ROOTS: dict[str, Path] = {
    "mot17": _REPO / "mot17" / "MOT17_yolox_dets",
    "sportsmot": _REPO / "sportsmot" / "sportsmot_yolox_dets",
    "dancetrack": _REPO / "dancetrack" / "dancetrack_yolox_dets",
}
...
def run_yolox_submit(
    ...
    *,
    ...
    for det_file in sorted(det_root.glob("*.txt")):
        tracker.reset()
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--dataset", choices=sorted(DATASET_DET_ROOTS), required=True)
    ...
    det_root = DATASET_DET_ROOTS[args.dataset] / args.split
    if not det_root.is_dir():
        print(f"Missing detections: {det_root}", file=sys.stderr)
        return 1
